...
rofiTheme="rofi -theme $HOME/.config/rofi/themes/rounded-gray-dark.rasi"
...
chosen_network=$(echo -e "$toggle\n$wifi_list" | uniq -u | $rofiTheme -dmenu -i -selected-row 1 -p "SSID Wi-Fi: " )
...
if [ -z "$chosen_network" ]; then
    ...
    if [[ "$chosen_network" =~ "ON" ]]; then
        ...
        chosen_action=$(echo -e "${options_formatted}" | $rofiTheme -dmenu -i -p "¿Qué acción deseas realizar?")
